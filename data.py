...
def Possible_quadrants(M, i, j, rows ,columns):
    ...
    if i + 1 < rows:
        pos.append(M[i + 1][j])
        if j + 1 < columns: pos.append(M[i + 1][j + 1])
    if j + 1 < columns:
        pos.append(M[i][j+1])
...
'''
Given the distance that defines the graph (1000 if no distance is given), calculates
the number of quadrants the theorical division of the map would have, and depending
on that, it executes one of the two algorithms implemented above for creating
the graph.
'''
def Graph(dist = 1000):
